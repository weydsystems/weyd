...
$count4 = $apps4->rowCount();
  ...
  $da =$pdo->prepare("SELECT * FROM ads LEFT JOIN ads_uploads ON ads.ads_id = ads_uploads.ads_id WHERE ads.ads_id='$adsname'");
  $da->execute(array($adsname));
  while($br = $da->fetch(PDO::FETCH_ASSOC)){
      ...
      $im ="/uploads/".$br['file'];
          ...
          $im1 ="/uploads/".$br['file1'];

              unlink($im1);

              $im2 ="/uploads/".$br['file2'];
                  unlink($im2);
