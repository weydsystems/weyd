...
<?php foreach($rows4 AS $row) {?>
  ...
  <h5 class="add-title"><a href="ads-details.php?at=<?php echo urlencode($row['ads_title']);?>&token=<?php echo urlencode($row['ads_id']);?>&kk=<?php echo base64_encode(openssl_random_pseudo_bytes(100)).".php";?>"><?php echo"<span style='color:#16A085; font-size:20px;'>".$row['ads_title'].'</span><br/>'."<span style='color:#E74C3C;'>";?> </a></h5>
  <span class="info-row">
...
<span style="color:red;"class="tooltipHere" data-toggle="tooltip" data-placement="right" title="<?php echo$row['ads_title'];?>"><?php echo$row['ads_condition'];?></span>
     <br/><span class="date"><i class=" icon-clock"> </i> Today 1:21 pm </span> - <span class="category">Electronics </span>- <span class="item-location"><i class="fa fa-map-marker"></i> London </span> </span></div>
  </div>
